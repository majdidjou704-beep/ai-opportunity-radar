...
function detectContext(q) {
  ...
  return {
    ...
    entreprise:
      /créer|creer|création|creation|lancer|ouvrir|entreprise|société|societe|activité|activite/.test(s),

    statut:
      /statut|forme juridique|micro|micro-entreprise|microentreprise|indépendant|independant|\bei\b|entreprise individuelle/.test(s),
    ...
    titreSejour:
      /titre de séjour|titre de sejour|carte de séjour|carte de sejour/.test(s),

    anef:
      /anef/.test(s),

    renouvellement:
      /renouvellement|renouveler|expire|expiration|fin de validité|fin de validite/.test(s),

    premiereDemande:
      /première demande|premiere demande|premier titre|première carte|premiere carte|je viens d'arriver|je viens d arriver/.test(s),
...
function actions(q, c) {
  ...
  if (c.immigration && c.recepisse) {
    ...
    a.push(
      "Vérifier si la demande de titre de séjour a déjà été déposée et quel document ou accusé de réception a été remis."
    );
...
/* =========================================================
   DIALOGUE PROGRESSIF V8.6
========================================================= */

function historiqueTexte(historique) {
  if (!Array.isArray(historique)) return "";

  return historique
    .slice(-12)
    .map(item => {
      const question = texte(item?.question, 3000);
      const reponse = texte(item?.reponse, 5000);

      if (!question && !reponse) return "";

      return `Question GouRare AI : ${question}\nRéponse utilisateur : ${reponse}`;
    })
    .filter(Boolean)
    .join("\n\n");
}

function prochaineQuestion(questionInitiale, reponseUtilisateur, c, historique) {
  const q = texte(questionInitiale, 12000).toLowerCase();
  const r = texte(reponseUtilisateur, 12000).toLowerCase();

  if (c.recepisse || c.immigration) {
    const dejaDepose =
      /oui|déposé|depose|déjà fait|deja fait|j'ai fait|j ai fait|demande faite|dossier déposé|dossier depose/.test(
        r
      );

    const pasDepose =
      /non|pas encore|jamais|aucune demande|je n'ai pas|je n ai pas/.test(r);

    const historiqueComplet = historiqueTexte(historique).toLowerCase();
    ...
    if (
      /type de demande|première demande|premiere demande|renouvellement|changement de situation|demande d'asile/.test(
        historiqueComplet
      )
    ) {
      return "Quel document avez-vous actuellement en votre possession concernant votre séjour en France ?";
    }
  ...
  if (c.entreprise) {
    if (!c.statut) {
      return "Quelle activité ou quels services souhaitez-vous proposer exactement ?";
    }

    if (c.statut) {
      return "Avez-vous déjà choisi une forme juridique pour votre activité, ou souhaitez-vous que GouRare AI vous aide à la comparer ?";
    }
  }

  if (c.travail) {
    return "Quel type d'emploi recherchez-vous exactement et dans quelle ville ou zone souhaitez-vous travailler ?";
  }

  if (c.social) {
    return "Quelle aide ou quel problème social souhaitez-vous résoudre exactement ?";
  }
  ...
  if (c.juridique) {
    return "Quel est précisément le problème juridique ou le document concerné ?";
  }

  if (c.fiscalite) {
    return "Êtes-vous concerné en tant que particulier, salarié, indépendant ou entreprise ?";
  }

  return "Pouvez-vous préciser votre situation et ce que vous souhaitez obtenir concrètement ?";
}
...
Les choix de parcours, de situation ou de profil dans l'interface sont uniquement des éléments de navigation.
Ils ne prouvent jamais un statut juridique, un titre de séjour, un droit, une nationalité, une situation irrégulière, une éligibilité ou une obligation.
...
Si une information importante manque, pose UNE seule question ciblée à la fois.

Ne pose pas une liste de questions dans la même réponse.

Quand une question a déjà été posée et que l'utilisateur y répond, utilise sa réponse pour réduire progressivement les possibilités.
...
Ne demande jamais à l'utilisateur s'il a "déposé une demande de récépissé".
La formulation correcte est :
"Avez-vous déjà déposé une demande de titre de séjour ?"
...
→ Poser la question la plus importante
→ Vérifier
→ Orienter
→ Donner la prochaine action
`;
}
...
async function analyserImage(env, image, demande = "") {
  ...
  const r = await env.IA.run(MODEL_VISION, {
    messages: [
      {
        role: "system",
        content:
          "Analyse uniquement ce qui est réellement visible ou lisible. Ne devine jamais un texte illisible."
      },
      {
        role: "user",
        content:
...
async function transcrireAudio(env, audio, langue = "fr") {
  ...
  const r = await env.IA.run(MODEL_AUDIO, {
...
async function analyserQuestion(env, q, options = {}) {
  const questionInitiale =
    texte(options.questionInitiale || q, 12000);

  const reponseUtilisateur =
    texte(options.reponseUtilisateur || "", 12000);

  const historique =
    Array.isArray(options.historique)
      ? options.historique
      : [];

  const profil =
    texte(options.profil || "", 100);

  const situation =
    texte(options.situation || "", 200);

  const etape =
    Number.isFinite(Number(options.etape))
      ? Number(options.etape)
      : 0;

  const contexte = detectContext(
    `${questionInitiale}\n${reponseUtilisateur}\n${historiqueTexte(historique)}`
  );
  ...
  if (contexte.message) {
    ...
    return {
      ...
      orientation:
        orientation ||
        "Le contenu a été reçu. Vérifiez que la réponse préparée correspond bien au message fourni.",
      confirmed: [],
      toVerify: [],
      recommendations: [],
      actions: [],
      ...
      professional: [],
      nextAction:
        "Vérifier le contenu final avant de l'envoyer.",
      questionSuivante: "",
      sources: []
    };
  }
...
Commence par expliquer brièvement ce que tu as compris.

Ne donne pas encore une conclusion juridique définitive.

Explique pourquoi une information supplémentaire est nécessaire.

Puis pose UNE SEULE question, correspondant exactement à la prochaine étape.
`,
      1000
    );
  } else {
    orientation = await askAI(
      env,
      `
Nous sommes dans une conversation progressive avec l'utilisateur.
...
1. comprendre la réponse donnée ;
2. mettre à jour progressivement la compréhension de la situation ;
3. ne pas répéter une question déjà résolue ;
4. ne pas inventer de droit ;
5. ne pas conclure juridiquement sans vérification ;
6. poser UNE SEULE question supplémentaire si une information importante manque.

Réponds en français.

Commence par une courte reformulation de ce que tu as compris.

Ensuite pose une seule question utile.
`,
      1000
    );
  }
  ...
  if (!orientation) {
    orientation =
      contexte.recepisse
        ? "Votre demande concerne un récépissé lié à une démarche de séjour. Pour vous orienter correctement, il faut d'abord identifier la procédure concernée."
        : "GouRare AI a identifié votre demande et va déterminer progressivement les informations nécessaires avant de proposer une démarche.";
  }

  const besoinQuestion =
    !contexte.message &&
    !!questionSuivante;
  ...
  return {
    ...
    toVerify: [
      ...(contexte.recepisse
        ? [
            "Le type exact de procédure : première demande, renouvellement, changement de situation, demande d'asile ou autre."
          ]
        : []),

      ...(contexte.recepisse
        ? [
            "Savoir si la demande de titre de séjour a déjà été déposée et quel document a été reçu après le dépôt."
          ]
        : []),
    ...
    nextAction:
      besoinQuestion
        ? "Répondez à la question suivante pour permettre à GouRare AI de préciser votre situation."
        : nextActionFallback(
            `${questionInitiale}\n${reponseUtilisateur}`,
            contexte
          ),
...
function nextActionFallback(q, c) {
  if (c.immigration && c.recepisse) {
    return "Après avoir identifié la procédure exacte, vérifier la démarche officielle correspondant à votre situation.";
  }
...
<button class="secondary back" id="retour">
← Retour
</button>
...
<button class="secondary back" id="retourOutil">
← Retour
</button>
...
function afficher(d){
  ...
  if(d.questionSuivante){

    h +=
      "<div class='question-next'>" +
      "<strong>❓ Question suivante</strong>" +
      "<div>" +
      esc(d.questionSuivante) +
      "</div>" +
      "<div class='actions'>" +
      "<button id='repondreQuestion'>Répondre</button>" +
      "</div>" +
      "</div>";
  ...
  if(bouton){
    ...
    function(){
      ...
      status.textContent =
        "Répondez à la question de GouRare AI.";
